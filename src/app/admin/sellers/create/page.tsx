...
export default function CreateSellerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Errore nella creazione del venditore');
      }

      setNotification({ type: 'success', message: 'Venditore creato con successo' });

      // Redirect dopo 1.5 secondi
      setTimeout(() => {
        router.push(`/admin/sellers/${data.sellerId}`);
      }, 1500);
  ...
  return (
    <AdminLayout
      ...
      subtitle="Aggiungi un nuovo venditore"
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md p-6 space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Creazione...' : 'Crea Venditore'}
            </button>
          </div>
        </form>
      </div>
