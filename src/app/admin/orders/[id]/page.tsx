...
export default function AdminOrderDetailsPage({
  ...
  const handleReviewRequest = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/orders/${orderId}/request-review`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Aggiorna i dati dell'ordine con il nuovo contatore
      if (orderDetails) {
        setOrderDetails({
          ...orderDetails,
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              {/* Bottone Richiesta Recensione - Solo se consegnato e NON esiste già feedback */}
              {orderDetails.shippingStatus === 'delivered' && !hasFeedback && !feedbackLoading && (
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 sm:p-6 border border-purple-200 shadow-lg">
                  ...
                  {(orderDetails.reviewRequestCount || 0) > 0 && (
                    <div className="bg-white rounded-lg p-4 mb-4 border border-purple-200">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="flex items-center justify-between sm:justify-start gap-2">
                          ...
                          <span className="inline-flex items-center justify-center w-8 h-8 bg-purple-500 text-white text-sm font-bold rounded-full">
                            {orderDetails.reviewRequestCount}
                          </span>
                        </div>
                        {orderDetails.lastReviewRequestDate && (
                          <div className="text-sm text-purple-600">
                            <span className="font-medium">Ultimo invio:</span>{' '}
                            {new Date(orderDetails.lastReviewRequestDate).toLocaleString('it-IT', {
                              day: '2-digit',
                              month: 'short',
                              year: 'numeric',
                              hour: '2-digit',
                      ...
                      {/* Timer 24h */}
                      {orderDetails.lastReviewRequestDate && (() => {
                        const hoursSinceLastRequest = (Date.now() - new Date(orderDetails.lastReviewRequestDate).getTime()) / (1000 * 60 * 60);
                  ...
                  <div className="bg-purple-100/50 border border-purple-200 rounded-lg p-4 mb-4">
                    <p className="text-purple-800 text-sm">
                      ...
                      {orderDetails.customer?.phone && ' e un messaggio WhatsApp'}
                  ...
                  <button
                    ...
                    disabled={isRequestingReview || (() => {
                      if (!orderDetails.lastReviewRequestDate) return false;
                      const hoursSinceLastRequest = (Date.now() - new Date(orderDetails.lastReviewRequestDate).getTime()) / (1000 * 60 * 60);
